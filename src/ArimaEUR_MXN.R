...
# Read the data
data <- read.csv("dataexchanges/data_EUR_MXN.csv", header = TRUE)
head(data)
...
plot(data$Date, data$Price, type="l", col="red",
     xlab="Date", ylab="EUR MXN",
     main="Official EUR MXN Time")
...
mean_EURMXN_per_year <- aggregate(data$Price, 
                                  by = list(data$Year), 
                                  FUN = mean, na.rm = TRUE)
colnames(mean_EURMXN_per_year) <- c("Year", "Mean_Official_EURMXN")

# Afficher les moyennes par année

print(mean_EURMXN_per_year)


# Calculer la moyenne générale du prix officiel LME
mean_official_ask_EURMXN <- mean(data$Price, na.rm = TRUE)

# Afficher les moyennes générales

print(paste("Moyenne générale du prix officiel EUR MXN: ", mean_official_ask_EURMXN))

library(urca)



kpss_test_EURMXN <- ur.kpss(data$Price)
summary(kpss_test_EURMXN)

# Plot the variables
plot(Price, main = "EUR to MXN Exchange Rate", ylab = "Price", xlab = "Time")
...
mae_df <- data.frame(Model = paste("Model", 1:22), MAE = mae)
...
write.csv(forecast$mean, file = "data/forecasted_valuesEUR_MXN.csv")
daforcasted <- read.csv("data/forecasted_valuesEUR_MXN.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelEUR_MXN.csv", header = TRUE)
print(daActuel)

daActuel_values <- as.numeric(gsub(",", ".", daActuel$Price))

# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual EUR to MXN Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for EUR to MXN Exchange Rate")
