...
# Read the data
data <- read.csv("dataexchanges/data_RON_RSD.csv", header = TRUE)
head(data)
...
write.csv(forecast$mean, file = "data/forecasted_valuesRON_RSD.csv")
daforcasted <- read.csv("data/forecasted_valuesRON_RSD.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelRON_RSD.csv", header = TRUE)
print(daActuel)

daActuel_values <- as.numeric(gsub(",", ".", daActuel$Price))

# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual RON to RSD Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for RON to RSD Exchange Rate")
