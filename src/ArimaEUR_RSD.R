...
# Read the data
data <- read.csv("dataexchanges/data_EUR_RSD.csv", header = TRUE)
head(data)
...
models <- list(
      ...
      Arima(data[,2], order = c(1, 1, 5)),
      Arima(data[,2], order = c(1, 1, 6)),
      Arima(data[,2], order = c(1, 1, 7)),
      Arima(data[,2], order = c(1, 1, 8)),
      Arima(data[,2], order = c(1, 1, 9)),
      Arima(data[,2], order = c(1, 1, 10)),
      ...
      Arima(data[,2], order = c(2, 1, 5)),
      Arima(data[,2], order = c(2, 1, 6)),
      Arima(data[,2], order = c(2, 1, 7)),
      Arima(data[,2], order = c(2, 1, 8)),
      Arima(data[,2], order = c(2, 1, 9)),
      Arima(data[,2], order = c(2, 1, 10))
)
...
write.csv(forecast$mean, file = "data/forecasted_valuesEUR_RSD.csv")
daforcasted <- read.csv("data/forecasted_valuesEUR_RSD.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelEUR_RSD.csv", header = TRUE)
print(daActuel)

daActuel_values <- as.numeric(gsub(",", ".", daActuel$Price))

# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual EUR to RSD Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for EUR to RSD Exchange Rate")
