...
Arima(data[,3] , order = c(6, 1, 0))
Arima(data[,3] , order = c(5, 1, 0))
Arima(data[,3] , order = c(4, 1, 0))
Arima(data[,3] , order = c(0, 1, 6))
Arima(data[,3], order = c(0, 1, 5))
Arima(data[,3] , order = c(0, 1, 4))
Arima(data[,3], order = c(6, 1, 4))
Arima(data[,3] , order = c(5, 1, 4))
Arima(data[,3], order = c(4, 1, 4))
Arima(data[,3] , order = c(6, 1, 5))
Arima(data[,3] , order = c(5, 1, 5))
Arima(data[,3], order = c(4, 1, 5))
Arima(data[,3], order = c(6, 1, 6))
Arima(data[,3] , order = c(5, 1, 6))
Arima(data[,3], order = c(4, 1, 6))
# Fit ARIMA models
models <- list(
      Arima(data[,3] , order = c(6, 1, 0)),
      Arima(data[,3] , order = c(5, 1, 0)),
...
fit <- Arima(data[,3]   ,order = c(6, 1, 4 ) )
...
forecast <- forecast(fit    , h = 18)


# Print the forecast
print(forecast)
plot(forecast, main ="ARIMA Forecast for Alum") 

# Save forecasted values into a CSV file
write.csv(forecast$mean, file = "data/forecasted_valuesAlum.csv")
daforcasted <- read.csv("data/forecasted_valuesAlum.csv" , header = TRUE  )
print(daforcasted[,2])

daActuel <- read.csv("data/dataActuelAlum.csv", header = TRUE , sep = ";")
print(daActuel)
...
# Ajout des valeurs prévues au plot
lines(daforcasted[,2], col = "red")

# Ajout de la légende avec une taille de police plus petite et placée en bas
