...
# URL de base pour l'aluminium
base_url <- 'https://www.westmetall.com/en/markdaten.php?action=table&field=LME_Al_cash'
...
# Nom du fichier CSV
file_name <- "C:/Users/HP/Downloads/Predicting_Metal_Cost_with_R-master-20240318T152350Z-001/Predicting_Metal_Cost_with_R-master/data/aluminium_pricesLME.csv"
...
scrape_year <- function(year_id) {
      ...
      table <- table %>%
            mutate(date = dmy(date),
                   price_aluminium_LME = as.numeric(gsub(",", "", `LME Aluminium Cash-Settlement`))) %>%
            filter(!is.na(date)) %>%
            select(date, price_aluminium_LME)
