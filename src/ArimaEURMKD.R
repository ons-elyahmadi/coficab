...
# Read the data
data <- read.csv("dataexchanges/data_EUR_MKD.csv", header = TRUE)
head(data)
...
write.csv(forecast$mean, file = "data/forecasted_valuesEUR_MKD.csv")
daforcasted <- read.csv("data/forecasted_valuesEUR_MKD.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelEUR_MKD.csv", header = TRUE)
print(daActuel)

daActuel_values <- as.numeric(gsub(",", ".", daActuel$Price))

# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual EUR to MKD Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
