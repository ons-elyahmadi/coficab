...
barplot(merged_data_complete$Cushing..OK.WTI.Spot.Price.FOB..Dollars.per.Barrel.)
...
na_indices_lme <- which(is.na(merged_data$Official.Ask.LME))
...
for (i in na_indices_lme) {
      # Vérifier si la valeur précédente est disponible
      if (!is.na(merged_data$Official.Ask.LME[i - 1]) & !is.na(merged_data$Official.Ask.LME[i + 1])) {
            # Remplacer NA par la moyenne de la valeur précédente et de la valeur suivante
            merged_data$Official.Ask.LME[i] <- (merged_data$Official.Ask.LME[i - 1] + merged_data$Official.Ask.LME[i + 1]) / 2
      } else if (!is.na(merged_data$Official.Ask.LME[i + 1]) & is.na(merged_data$Official.Ask.LME[i - 1])) {
            # Si la valeur précédente est manquante mais la valeur suivante est disponible, remplacer par la valeur suivante
            merged_data$Official.Ask.LME[i] <- merged_data$Official.Ask.LME[i + 1]
      }
      else if (is.na(merged_data$Official.Ask.LME[i + 1]) & !is.na(merged_data$Official.Ask.LME[i - 1])) {
      # Si la valeur précédente est manquante mais la valeur suivante est disponible, remplacer par la valeur suivante
      merged_data$Official.Ask.LME[i] <- merged_data$Official.Ask.LME[i - 1]
