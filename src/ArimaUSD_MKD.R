...
# Read the data
data <- read.csv("dataexchanges/data_USD_MKD.csv", header = TRUE)
...
plot(data$Date, data$Price, type="l", col="red",
     xlab="Date", ylab="USD RON",
     main="Official USD RON Time")
...
mean_USD_MKD_per_year <- aggregate(data$Price, 
                                  by = list(data$Year), 
                                  FUN = mean, na.rm = TRUE)
colnames(mean_USD_MKD_per_year) <- c("Year", "Mean_Official_USD_MKD")

# Afficher les moyennes par année

print(mean_USD_MKD_per_year)


# Calculer la moyenne générale du prix officiel LME
mean_official_ask_USD_MKD <- mean(data$Price, na.rm = TRUE)

# Afficher les moyennes générales

print(paste("Moyenne générale du prix officiel USD RON: ", mean_official_ask_USD_MKD))

library(urca)



kpss_test_USD_MKD <- ur.kpss(data$Price)
summary(kpss_test_USD_MKD)
# Plot the variables
plot(Price, main = "USD to RON Exchange Rate", ylab = "Price", xlab = "Time")
...
models <- list(
      ...
      Arima(data[,2], order = c(5, 1, 0)),
      Arima(data[,2], order = c(5, 1, 1)),
      Arima(data[,2], order = c(5, 1, 2)),
      Arima(data[,2], order = c(5, 1, 3)),
      Arima(data[,2], order = c(5, 1, 4)),
      Arima(data[,2], order = c(5, 1, 5)),
      Arima(data[,2], order = c(5, 1, 6)),
      Arima(data[,2], order = c(5, 1, 7)),
      Arima(data[,2], order = c(5, 1, 8)),
      Arima(data[,2], order = c(5, 1, 9)),
      Arima(data[,2], order = c(5, 1, 10)),
      Arima(data[,2], order = c(6, 1, 0)),
      Arima(data[,2], order = c(6, 1, 1)),
      Arima(data[,2], order = c(6, 1, 2)),
      Arima(data[,2], order = c(6, 1, 3)),
      Arima(data[,2], order = c(6, 1, 4)),
      Arima(data[,2], order = c(6, 1, 5)),
      Arima(data[,2], order = c(6, 1, 6)),
      Arima(data[,2], order = c(6, 1, 7)),
      Arima(data[,2], order = c(6, 1, 8)),
      Arima(data[,2], order = c(6, 1, 9)),
      Arima(data[,2], order = c(6, 1, 10))
)
...
write.csv(forecast$mean, file = "data/forecasted_valuesUSD_MKD.csv")
daforcasted <- read.csv("data/forecasted_valuesUSD_MKD.csv", header = TRUE)
print(daforcasted[,2])
# Read the actual data
daActuel <- read.csv("data/actual_data_USD_MKD.csv", header = TRUE)
print(daActuel)

daActuel_values <- as.numeric(gsub(",", ".", daActuel$Price))

# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual EUR to USD Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for  USD_MXN Exchange Rate")
