...
# Read the data
data <- read.csv("dataexchanges/data_EUR_USD.csv", header = TRUE)
head(data)
...
mean_EURUSD_per_year <- aggregate(data$Price, 
                                  by = list(data$Year), 
                                  FUN = mean, na.rm = TRUE)
colnames(mean_EURUSD_per_year) <- c("Year", "Mean_Official_EURUSD")

# Afficher les moyennes par année

print(mean_EURUSD_per_year)


# Calculer la moyenne générale du prix officiel LME
mean_official_ask_EURUSD <- mean(data$Price, na.rm = TRUE)

# Afficher les moyennes générales

print(paste("Moyenne générale du prix officiel EUR USD: ", mean_official_ask_EURUSD))

library(urca)



kpss_test_EURUSD <- ur.kpss(data$Price)
summary(kpss_test_EURUSD)
# Plot the variables
plot(Price, main = "EUR to USD Exchange Rate", ylab = "Price", xlab = "Time")
...
write.csv(forecast$mean, file = "data/forecasted_valuesEUR_USD.csv")
daforcasted <- read.csv("data/forecasted_valuesEUR_USD.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelEUR_USD.csv", header = TRUE)
print(daActuel)
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for EUR to USD Exchange Rate")
