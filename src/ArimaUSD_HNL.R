...
# Read the data
data <- read.csv("dataexchanges/data_USD_HNL.csv", header = TRUE)
head(data)
...
write.csv(forecast$mean, file = "data/forecasted_valuesUSD_HNL.csv")
daforcasted <- read.csv("data/forecasted_valuesUSD_HNL.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelUSD_HNL.csv", header = TRUE)
print(daActuel)

daActuel_values <- as.numeric(gsub(",", ".", daActuel$Price))

# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual USD to HNL Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for USD to HNL Exchange Rate")
