...
# Read the data
data <- read.csv("dataexchanges/data_EUR_TND.csv", header = TRUE)
head(data)
...
plot(data$Date, data$Price, type="l", col="red",
     xlab="Date", ylab="EUR TND",
     main="Official EUR TND Time")
...
mean_EURTND_per_year <- aggregate(data$Price, 
                                  by = list(data$Year), 
                                  FUN = mean, na.rm = TRUE)
colnames(mean_EURTND_per_year) <- c("Year", "Mean_Official_EURTND")

# Afficher les moyennes par année

print(mean_EURTND_per_year)


# Calculer la moyenne générale du prix officiel LME
mean_official_ask_EURTND <- mean(data$Price, na.rm = TRUE)

# Afficher les moyennes générales

print(paste("Moyenne générale du prix officiel EUR TND: ", mean_official_ask_EURTND))

library(urca)



kpss_test_EURTND <- ur.kpss(data$Price)
summary(kpss_test_EURTND)
# Plot the variables
plot(Price, main = "EUR to TND Exchange Rate", ylab = "Price", xlab = "Time")
...
write.csv(forecast$mean, file = "data/forecasted_valuesEUR_TND.csv")
daforcasted <- read.csv("data/forecasted_valuesEUR_TND.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelEUR_TND.csv", header = TRUE)
print(daActuel)

daActuel_values <- as.numeric(gsub(",", ".", daActuel$Price))

# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual EUR to TND Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for EUR to TND Exchange Rate")
