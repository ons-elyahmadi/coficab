...
# Read the data
data <- read.csv("dataexchanges/data_EUR_MAD.csv", header = TRUE)
head(data)
...
plot(data$Date, data$Price, type="l", col="red",
     xlab="Date", ylab="EUR MAD",
     main="Official EUR MAD Time")
...
mean_EURMAD_per_year <- aggregate(data$Price, 
                                  by = list(data$Year), 
                                  FUN = mean, na.rm = TRUE)
colnames(mean_EURMAD_per_year) <- c("Year", "Mean_Official_EURMAD")

# Afficher les moyennes par année

print(mean_EURMAD_per_year)


# Calculer la moyenne générale du prix officiel LME
mean_official_ask_EURMAD <- mean(data$Price, na.rm = TRUE)

# Afficher les moyennes générales

print(paste("Moyenne générale du prix officiel EUR MAD: ", mean_official_ask_EURMAD))

library(urca)



kpss_test_EURMAD <- ur.kpss(data$Price)
summary(kpss_test_EURMAD)
# Plot the variables
plot(Price, main = "EUR to MAD Exchange Rate", ylab = "Price", xlab = "Time")
...
aic_df <- data.frame(Model = paste("Model", 1:10), AIC = aic)
...
bic_df <- data.frame(Model = paste("Model", 1:10), BIC = bic)
...
rmse_df <- data.frame(Model = paste("Model", 1:10), RMSE = rmse)
...
likelihood_df <- data.frame(Model = paste("Model", 1:10), Likelihood = likelihood)
...
r_squared_df <- data.frame(Model = paste("Model", 1:10), R_squared = r_squared)
...
schwarz_criterion_df <- data.frame(Model = paste("Model", 1:10), Schwarz_Criterion = schwarz_criterion)
...
se_regression_df <- data.frame(Model = paste("Model", 1:10), SE_Regression = se_regression)
...
mae_df <- data.frame(Model = paste("Model", 1:10), MAE = mae)
...
mse_df <- data.frame(Model = paste("Model", 1:10), MSE = mse)
...
# Fit ARIMA with best model with exogenous variable (example: ARIMA(3,1,3))
...
write.csv(forecast$mean, file = "data/forecasted_valuesEUR_MAD.csv")
daforcasted <- read.csv("data/forecasted_valuesEUR_MAD.csv", header = TRUE)
print(daforcasted[,2])

# Read the actual data
daActuel <- read.csv("data/dataactuelEUR_MAD.csv", header = TRUE)
print(daActuel)
...
# Plot actual values
plot(daActuel_values, type = "l", col = "blue", xlab = "Time", ylab = "Price", main = "Actual EUR/MAD Exchange Rates vs Forecasted Values", ylim = c(min(daActuel_values, na.rm = TRUE), max(daActuel_values, na.rm = TRUE)))
...
mae <- mean(abs(daforcasted[,2] - daActuel_values))
mse <- mean(( daforcasted[,2] - daActuel_values)^2)
...
rmse <- sqrt(mean(( daforcasted[,2] - daActuel_values)^2))

# Calculate Mean Absolute Percentage Error (MAPE)
mape <- mean(abs(( daforcasted[,2] - daActuel_values) / daActuel_values)) * 100
...
# Plot forecast
plot(forecast, main = "ARIMA Forecast for EUR/MAD Exchange Rate")
