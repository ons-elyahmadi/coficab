...
plot(data$Dateoil, data$COMEX, type="l", col="red",
     xlab="Date", ylab="Official COMEX",
     main="Official Ask Comex Over Time")
...
mean_comex_per_year <- aggregate(data$COMEX, 
                               by = list(data$Year), 
                               FUN = mean, na.rm = TRUE)
colnames(mean_comex_per_year) <- c("Year", "Mean_Official_Comex")

# Afficher les moyennes par année
print(mean_wti_per_year)
print(mean_comex_per_year)
...
mean_official_ask_comex <- mean(data$COMEX, na.rm = TRUE)

# Afficher les moyennes générales
print(paste("Moyenne générale du prix au comptant WTI: ", mean_wti_spot_price))
print(paste("Moyenne générale du prix officiel comex : ", mean_official_ask_comex))
...
kpss_test_comex <- ur.kpss(data$COMEX)
summary(kpss_test_comex)
